...
// Interfaces internas para el mapeo de BD
...
export class AuditoriaService {
  static async obtenerCambiosPaginados(
    ...
    try {
      ...
      const resultTotal = await sql<CountResult>(
        queryTotalReescrita,
        paramsCount
      );
      ...
      const result = await sql<LogAuditoriaRow>(query, paramsData);
  ...
  static async obtenerNotificacionesPaginadas(
    ...
    try {
      ...
      const result = await sql<LogNotificacionRow>(query, paramsData);
      ...
      const datos = result.rows.map((row) => ({
        ...
        error: row.mensaje_error || undefined,
      }));
